...
When repository changes are required, clone the upstream repository into `./repo`, add the bot fork as the `fork` remote, and work there. For an existing PR, obtain its head branch with `gh pr view {number} --repo {repo} --json headRefName --jq '.headRefName'`, then fetch that branch from the fork. The task directory itself provides isolation; do not use shared clones, caches, or git worktrees from another path.
...
4. Determine what action is being requested:
   ...
   - **Fix in an existing PR** — treat like the `pr-feedback` workflow. Fetch the PR branch from the **fork** (not upstream) into the task-local clone, then push changes to the fork.
...
6. If code changes are needed, clone the repository as described in the Environment section. Always add the fork remote.
...
12. If the request is ambiguous or you lack information to proceed safely, post a comment on the thread asking for clarification, then **unassign yourself** and exit. Do not block waiting for a reply — you are non-interactive.
